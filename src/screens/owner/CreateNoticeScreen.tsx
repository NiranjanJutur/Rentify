...
import { Alert, SafeAreaView, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View, ActivityIndicator, Platform } from 'react-native';
...
export const CreateNoticeScreen = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
          alert('Notice Published! Residents can now see this in their portal.');
          navigation.goBack();
      } else {
        Alert.alert('Notice Published', 'Residents can now see this notice.', [
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        <View style={styles.content}>
            ...
            <View style={styles.chipRow}>
                ...
                >
                    <Text style={[styles.priorityChipText, priority === p.value && { color: '#fff' }]}>{p.label}</Text>
                </TouchableOpacity>
                ))}
            </View>
            ...
            <View style={styles.footer}>
                <RentifyButton 
                title={saving ? 'Publishing Announcement...' : 'Publish Notice'} 
                onPress={handleSave} 
                disabled={saving}
            ...
            <View style={styles.previewCard}>
                ...
                <Text style={styles.previewText}>This notice will be instantly visible to all tenants registered to your first property.</Text>
            </View>
        </View>
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  title: { 
    ...
    fontSize: 32, fontWeight: '700', color: theme.colors.onSurface 
  },
  subtitle: { 
  ...
  label: { 
    fontFamily: theme.typography.label.fontFamily, 
    fontSize: 12, fontWeight: '600', color: theme.colors.onSurfaceVariant, 
  ...
  previewText: {
      flex: 1, fontSize: 13, color: theme.colors.onSurfaceVariant,
      lineHeight: 18,
  }
});
